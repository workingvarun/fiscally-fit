...
export default function BudgetsPage() {
  ...
  const handleFormSubmit = () => {
    if (!amount.trim() || parseFloat(amount) <= 0 || !selectedCategoryId || !monthYear) {
      toast({ title: "Error", description: "Amount, category, and month/year are required.", variant: "destructive" });
      return;
    }
    ...
    if (existing) {
      toast({ title: "Error", description: `A budget for ${getCategoryName(selectedCategoryId)} for ${formatMonthYear(monthYear)} already exists. Edit it instead.`, variant: "destructive", duration: 5000 });
      return;
    }
  ...
  const availableYears = Array.from(new Set([getYear(new Date()), ...budgets.map(b => parseInt(b.monthYear.split('-')[0]))])).sort();
  const availableMonths = Array.from({ length: 12 }, (_, i) => ({ value: (i + 1).toString().padStart(2, '0'), label: format(setMonth(new Date(), i), 'MMMM') }));


  // Filter budgets by selected monthYear for display
    ...
    <>
      ...
      <Card className="mb-6 shadow-md">
        <CardContent className="pt-6 flex flex-col sm:flex-row gap-4 items-center">
          <Label htmlFor="monthYearFilter" className="text-nowrap">View Budgets For:</Label>
        ...
        <Card className="shadow-lg">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center justify-center h-40 text-center">
              ...
              <h3 className="text-xl font-semibold mb-1">No Budgets Set for {formatMonthYear(monthYear)}</h3>
              <p className="text-muted-foreground mb-4">Create budgets to start tracking your goals.</p>
              <Button onClick={() => setIsFormOpen(true)}>
                <PlusCircle className="mr-2 h-4 w-4" /> Set Budget
              </Button>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg font-semibold flex items-center">
                     ...
                     <div className="flex space-x-1">
                        <Button variant="ghost" size="icon" onClick={() => openEditForm(budget)} aria-label="Edit budget">
                          <Edit3 className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive/80" onClick={() => handleDeleteBudget(budget.id)} aria-label="Delete budget">
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                  </div>
                  <CardDescription className="text-sm">
                    Budget for {formatMonthYear(budget.monthYear)}: ${budget.amount.toFixed(2)}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="mt-2 text-sm flex justify-between">
                    ...
                    <span className={isOverBudget ? "text-destructive font-medium" : "text-muted-foreground"}>
                      {isOverBudget 
                        ? `Over by $${(spentAmount - budget.amount).toFixed(2)}` 
      ...
      <Dialog open={isFormOpen} onOpenChange={(isOpen) => { if(!isOpen) closeForm(); else setIsFormOpen(true); }}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingBudget ? 'Update budget details.' : 'Set a spending goal for a category.'}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="budgetAmount" className="text-right">Amount</Label>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
             ...
             <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="budgetMonthYear" className="text-right">Month/Year</Label>
               <Select value={monthYear} onValueChange={setMonthYear} disabled={!!editingBudget}>
                ...
                <SelectContent>
                  ...
                  ).sort((a,b) => b.value.localeCompare(a.value)) // Sort descending by value "yyyy-MM"
                  .map(my => (
          ...
          <DialogFooter>
            ...
            <Button onClick={handleFormSubmit}>{editingBudget ? 'Save Changes' : 'Set Budget'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
